...
use rusqlite::types::{FromSql, ToSql, Value, ValueRef, FromSqlResult, ToSqlOutput, FromSqlError};
use model::*;
    ...
    fn store(&mut self, tx: &Transaction, mqtt: &Option<Sender<TypedAction>>) -> Option<u64> {
        match self.action.id {
            None => {
                ...
                tx.execute("INSERT INTO status_action (id, user, status, changed, public_changed) \
                             VALUES (?, ?, ?, ?, ?)",
...
impl DbStored for AnnouncementAction {
    fn store(&mut self, tx: &Transaction, _mqtt: &Option<Sender<TypedAction>>)
        -> Option<u64> {
        match self.action.id {
            None => {
                match self.method {
                    AnnouncementMethod::New =>
                        match self.aid {
                            None => {
                                let action_id = DbStoredTyped::store(&mut self.action, 1,
                                                                     tx).unwrap();
                                tx.execute("INSERT INTO announcement_action \
...
impl DbStored for PresenceAction {
    fn store(&mut self, tx: &Transaction, mqtt: &Option<Sender<TypedAction>>)
        -> Option<u64> {
        match self.action.id {
            None => {
                tx.execute("INSERT INTO action (time, type, note) VALUES (?, ?, ?)",
                ...
                for user in self.users.iter() {
                    if user.status != PresentUserStatus::Left {
                        tx.execute("INSERT INTO presence_action (id, user, since) VALUES (?, ?, ?)",
...
pub fn query(type_: QueryActionType, id: RangeExpr<IdExpr>, time: RangeExpr<i64>,
             count: u64, take: Take, con: &mut DbCon) -> Result<Vec<Box<Action>>, Error> {
    ...
    let actions_iter = stmt.query_map(&*params, |row| -> Box<Action> {
        match row.get(1) {
            0 => Box::new(status::get_by_id(row.get::<_, i64>(0) as u64, &con).unwrap()) as Box<Action>,
            1 => Box::new(announcements::get_by_id(row.get::<_, i64>(0) as u64, &con).unwrap()) as Box<Action>,
            2 => Box::new(presence::get_by_id(row.get::<_, i64>(0) as u64, &con).unwrap()) as Box<Action>,
